function [tt, y_val_nn_predict_final, y_real] = func_predict(data_original_filename,line_number)

line_number_str = num2str(line_number);
...
val_r_step_length = length(data_y) - 2 * half_input_wid;
...
%% adding half_input_wid
y_val_nn_predict_final = zeros(  size(y_val_nn_predict,1),  size(y_val_nn_predict,2) + 2*half_input_wid );
y_val_nn_predict_final(:,half_input_wid+1:end-half_input_wid) = y_val_nn_predict;
for d_i = 1:half_input_wid
    y_val_nn_predict_final(:,d_i) = y_val_nn_predict(:,1);
    y_val_nn_predict_final(:,end-d_i+1) = y_val_nn_predict(:,end);
end
